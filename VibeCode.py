...
class SafeiPhonePhotoReader(InterruptibleOperation):
    ...
    def download_file_safe(self, remote_path, local_path, file_info=None):
        ...
        try:
            ...
            # 分塊讀取文件
            chunk_size = 512 * 1024  # 512KB chunks (降低塊大小提高穩定性)
            downloaded_size = 0
            total_size = file_info.get('size', 0) if file_info else 0
            
            try:
                with self.afc.open(remote_path, 'rb') as remote_file:
                    with open(local_path, 'wb') as local_file:
                        while not self.is_stopped():
                            try:
                                ...
                                logger.error(f"讀取文件塊失敗: {read_error}")
                                break
                                
            except Exception as file_error:
                logger.error(f"打開文件失敗 {remote_path}: {file_error}")
                return False
            ...
            if os.path.exists(local_path):
                ...
                if total_size > 0 and actual_size != total_size:
                    logger.warning(f"文件大小不匹配 {os.path.basename(local_path)}: {actual_size} vs {total_size}")
